...
def get_stt_handler(
    ...
    elif module_kwargs.stt == "nemotron":
        from speech_to_speech.STT.nemotron_stt_handler import NemotronSTTHandler

        return NemotronSTTHandler(
            stop_event,
            queue_in=spoken_prompt_queue,
            queue_out=text_prompt_queue,
            setup_kwargs={"language": "hu"},
        )
    else:
        raise ValueError(
            "The STT should be either whisper, whisper-mlx, mlx-audio-whisper, faster-whisper, parakeet-tdt, paraformer, or nemotron."
        )
...
def get_tts_handler(
    ...
    if module_kwargs.tts == "chatTTS":
    ...
    elif module_kwargs.tts == "edge-tts":
        from speech_to_speech.TTS.edge_tts_handler import EdgeTTSHandler
        ...
        raise ValueError("The TTS should be either chatTTS, facebookMMS, pocket, kokoro, qwen3, edge-tts, or omnivoice")
